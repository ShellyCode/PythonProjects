...
#            possibilities = possibilities + ['open']               #pointless opening and closing.
    ...
    def do(self,action):
        '''action is a string indicating an action for the RobotWorld to do.
           These strings come from the 'allowedActions' method, and are part of the process of
    ...
    def visualize(self,serial_number):
        '''The blocks are colored according to their identity (exactly one color per block).
           These are persistent over time, so a block with a certain name in one frame of the
           animation will have the same color in all other frames.
           The triangles of the same color as a block indicate where the block's goal location is.
           The title 'cost=<number>' at the top indicates the cumulative cost of actions so far
           to bring the RobotWorld into its current state.
...
def graphsearch(queue,queue_modification,timeout,visualize=False):
    ...
    while len(queue) > 0:
        ...
        #print expanded.hand['location'],expanded.hand['held']
        if expanded.isGoal():
            tf = time.time()
...
def duplicateWorld(world,worlds):
    ...
    #This function is needed to define a more loose idea of equality,
    #so that worlds with different accumulated costs are considered equivalent.
    for w in worlds:
...
def breadthFirst(queue,visited):
    ...
    i,world=heappop(queue)
    if not world.isGoal():
        for action in world.allowedActions():
            neighbor=copy.deepcopy(world)
            neighbor.do(action)
            if not duplicateWorld(neighbor,visited):
                heappush(queue,(neighbor.cost,neighbor))
    return queue,world
...
def depthFirst(queue,visited):
    ...
    if not world.isGoal():
        for action in world.allowedActions():
            ...
            neighbor.do(action)
            if not duplicateWorld(neighbor,visited):
                heappush(queue,(neighbor.cost*(-1),neighbor))
    return queue,world
...
def bestFirst(queue,visited,heuristic):
    ...
    if not world.isGoal():
       # print i,world.hand['location'],world.hand['held']
        for action in world.allowedActions():
            neighbor=copy.deepcopy(world)
            neighbor.do(action)
            if not duplicateWorld(neighbor,visited):
...
#                    return queue,neighbor
                if world.hand['closed?']:
                    name=world.hand['held']
                    addworld=(heuristic(world.hand['location'],world.goal[name]),neighbor)
                else:
                    for block_name,block_coord in neighbor.blocks.iteritems():
                        if neighbor.goal[block_name]!=block_coord:
                            h=heuristic(neighbor.hand['location'],block_coord)+\
                               heuristic(block_coord,neighbor.goal[block_name])
                            addworld=(h,neighbor)
                heappush(queue,addworld)                           
                if not world.hand['held'] and neighbor.hand['held']:
                    break
    return queue,world
def tentative_cost(x,y):
...
def aStar(queue,visited,heuristic):
    ...
    if not world.isGoal():
        current_g_score=world.cost
        #print i,world.hand['location'],world.hand['held']
        for action in world.allowedActions():
            neighbor=copy.deepcopy(world)
            neighbor.do(action)
            if not duplicateWorld(neighbor,visited):
##                tentative_g_score=current_g_score+heuristic(world.hand['location'],neighbor.hand['location'])
##                if tentative_g_score>neighbor.cost:
##                    continue
                if neighbor.hand['closed?']:
                    name=neighbor.hand['held']
                    h=heuristic(neighbor.blocks[name],neighbor.goal[name])
                    g=neighbor.cost+tentative_cost(neighbor.blocks[name],neighbor.goal[name])
                    heappush(queue,(h+g,neighbor))
                else:
                    for block_name,block_coord in neighbor.blocks.iteritems():
                        if neighbor.goal[block_name]!=block_coord:
                                h=heuristic(neighbor.hand['location'],block_coord)+heuristic(block_coord,neighbor.goal[block_name])
                                g=neighbor.cost+tentative_cost(neighbor.hand['location'],block_coord)+tentative_cost(block_coord,neighbor.goal[block_name])
                                heappush(queue,(h+g,neighbor))
...
def Euclidean(start,end):
    return (sum([(start[i]-end[i])**2 for i in range(len(start))] ))**0.5
